...
compute_NMBs_ <- function(.effs, .costs, .ref = NULL, .interventions = NULL,
  ...
  if(!is.null(.ref)) {
    ...
    return(list(nmb = inmb, e.nmb = e.inmb, check = check, kstar = kstar,
                delta.effs = delta.effs, delta.costs = delta.costs,
...
compute_CEAFs_ <- function(.ceac, .nmb = NULL, .ref = NULL) {
  ...
  stopifnot('.ceac is not a tibble' = "data.frame" %in% class(.ceac))
