...
#'
#' @examples
identify_dominance <- function(.icer_data, .qalys = qalys, .costs = costs) {
...
#'
#' @examples
identify_e.dominance <- function(.icer_data, .qalys = qalys) {
  # Check if missing key columns and create them if so:
  .icer_data <- .icer_data %>%
    add_missing_columns(.x = .,
                        .characters = c("dominance", "icer_label"),
...
#'
#' @examples
compute_ICERs <- function(.icer_data, .qalys = qalys, .costs = costs,
  ...
  if(.incremental) {
    ...
    .icer_data <- .icer_data %>%
      ...
      mutate(
        ...
        icer_label = case_when(
          !is.na(icer) ~ paste("ICER reference is", .ref_name),
          TRUE ~ paste("ICER reference")),
        dominance = paste("Not checked")
      )
  }
...
e.dominance_wraper <- function(.x) {
  ...
  while (any("e.dominated" %in% (.x %>%
    ...
    .x <- .x %>%
      identify_e.dominance() %>%
      compute_ICERs() # ICER(s) for un-dominated/e.dominated
  }
...
#'
#' @examples
get_icers <- function(.icer_data, .effs = NULL, .costs = NULL, .ref = NULL,
                      .interventions = NULL, .incremental = TRUE) {
  ...
  if(!is.null(.icer_data)) {
    ...
    # Stop if .effs & .costs are not of class matrix or not equal:
    stopifnot('is e a matrix' = "matrix" %in% class(.effs),
              'is c a matrix' = "matrix" %in% class(.costs),
              'e and c dimensions' = dim(.effs) == dim(.costs))
    ...
    stop("Please supply costs and effects from PSA, each in a separate matrix, or a table with interventions' names, mean costs, and mean qalys")
  }
  ...
  if(!.incremental){
    ...
    icer_tmp <- icer_tmp %>%
      compute_ICERs(.ref = .ref, .incremental = .incremental)
